	...
	for index, row in changes.iterrows():
		if (row["newShared"]==False and row["newShift"]==1) or (row["newShared"]==True):
			if(row["id_emp"]!=empid):
				if(row["id_emp"]!=0):
					empid=row["id_emp"]
					oldshift =str(row["oldShift"])
				table+=f"""	 <tr>
 			   					<td style="text-align: center;">{row["oldSeat"]}</td>
 			   					<td style="text-align: center;">{'' if row["oldSeat"]== '' else oldshift[0]}</td>
...
def send_email(html):
	...
	payload = json.dumps({
	  ...
	  "toEmail": "manuel.diaz@here.com",
	  "plainTxt": "Go to layout tool to see the changes ",
	  "html": html
	})

	#"manuel.diaz@here.com,marco.a.salazar@here.com,ext-miguel.thompson@here.com,ext-paulina.mata@here.com,raquel.jimenez@here.com,fernando.delgado@here.com",
